...
if ( !class_exists( 'RPR_Activate' ) ) {
	class RPR_Activate {
		function __construct() {
			...
			add_filter( 'wpmu_welcome_user_notification', array( $this, 'filter_wpmu_welcome_user_notification' ), 10, 3 );
		...
		function filter_random_password( $password ) {
			...
			global $pagenow;
			if ( $pagenow == 'wp-activate.php' && $register_plus_redux->GetReduxOption( 'user_set_password' ) == TRUE ) {
				//WP method
		...
		function filter_wpmu_welcome_user_notification( $user_id, $password, $meta ) {
			global $register_plus_redux;
			if ( $register_plus_redux->GetReduxOption( 'disable_user_message_registered' ) == TRUE ) return FALSE;
			else return TRUE;
		}
		...
		function rpr_restore_signup_fields( $user_id, $password, $meta ) {
			...
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'first_name', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['first_name'] ) ) update_user_meta( $user_id, 'first_name', sanitize_text_field( $meta['first_name'] ) );
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'last_name', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['last_name'] ) ) update_user_meta( $user_id, 'last_name', sanitize_text_field( $meta['last_name'] ) );
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'url', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['user_url'] ) ) {
			...
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'aim', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['aim'] ) ) update_user_meta( $user_id, 'aim', sanitize_text_field( $meta['aim'] ) );
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'yahoo', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['yahoo'] ) ) update_user_meta( $user_id, 'yim', sanitize_text_field( $meta['yahoo'] ) );
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'jabber', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['jabber'] ) ) update_user_meta( $user_id, 'jabber', sanitize_text_field( $meta['jabber'] ) );
			if ( is_array( $register_plus_redux->GetReduxOption( 'show_fields' ) ) && in_array( 'about', $register_plus_redux->GetReduxOption( 'show_fields' ) ) && !empty( $meta['description'] ) ) update_user_meta( $user_id, 'description', wp_filter_kses( $meta['description'] ) );
			...
			foreach ( $redux_usermeta as $index => $meta_field ) {
				if ( current_user_can( 'edit_users' ) || !empty( $meta_field['show_on_registration'] ) ) {
					if ( !empty( $meta[$meta_field['meta_key']] ) ) $register_plus_redux->SaveMetaField( $meta_field, $user_id, $meta[$meta_field['meta_key']] );
				}
			}

			if ( $register_plus_redux->GetReduxOption( 'enable_invitation_code' ) == TRUE && !empty( $meta['invitation_code'] ) ) update_user_meta( $user_id, 'invitation_code', sanitize_text_field( $meta['invitation_code'] ) );

			/* filter_random_password replaces the random password with the password stored in meta
			if ( $register_plus_redux->GetReduxOption( 'user_set_password' ) == TRUE && !empty( $meta['password'] ) ) {
