<?php
if ( !class_exists( 'RPR_Admin_Menu_wip' ) ) {
	class RPR_Admin_Menu_wip {
		public function __construct() {
			add_action( 'admin_menu', array( $this, 'rpr_admin_menu' ), 10, 0 );

			add_action( 'admin_init', array( $this, 'rpr_register_settings' ), 10, 0 );

		}

		public function rpr_admin_menu() {
			add_menu_page( __( 'Register Plus Redux Features', 'register-plus-redux' ), 
				__( 'Register+Redux', 'register-plus-redux' ), 
				'manage_options', 
				'rpr-features', 
				array( $this, 'rpr_features_page' ) );
			add_submenu_page(
				'rpr-features', 
				__( 'Register Plus Redux - User Fields', 'register-plus-redux' ), 
				__( 'User Fields', 'register-plus-redux' ),
				'manage_options', 
				'rpr-fields',
				array( $this, 'rpr_fields_page' )
			);
			add_submenu_page(
				'rpr-features', 
				__( 'Register Plus Redux - Pages and Templates', 'register-plus-redux' ), 
				__( 'Pages and Templates', 'register-plus-redux' ),
				'manage_options', 
				'rpr-pages',
				array( $this, 'rpr_pages_page' )
			);
			add_submenu_page(
				'rpr-features', 
				__( 'Register Plus Redux - Messages', 'register-plus-redux' ), 
				__( 'Messages', 'register-plus-redux' ),
				'manage_options', 
				'rpr-messages',
				array( $this, 'rpr_messages_page' )
			);
			global $submenu;
			if ( isset( $submenu['rpr-features'] ) ) {
				$submenu['rpr-features'][0][0] = __( 'Features', 'register-plus-redux' );
			
			}			
//add_options_page( __( 'Register+Redux Settings', 'register-plus-redux' ), __( 'Register+Redux', 'register-plus-redux' ), 'manage_options', 'register-plus-redux-options', array( $this, 'rpr_options_page' ) );
...
public function rpr_register_settings() {
	register_setting( 'register-plus-redux', 'register_plus_redux_options_v2', array( $this, 'rpr_sanitize_options' ) );

	add_settings_section( 'rpr_features', NULL, NULL, 'rpr-features' );
	add_settings_field( 'verify_user_email', 'Email Verification', array( $this, 'rpr_feature_verify_user_email' ), 'rpr-features', 'rpr_features', array(
		__( 'Verify all new users email address...', 'register-plus-redux' ) ) ); 

	if ( !is_multisite() ) add_settings_section( 'rpr_form_options', __( 'Registration Form', 'register-plus-redux' ), array( $this, 'rpr_form_registration_options' ), 'rpr-features' );
	if ( is_multisite() ) add_settings_section( 'rpr_form_options', __( 'Signup Form', 'register-plus-redux' ), array( $this, 'rpr_form_signup_options' ), 'rpr-features' );
	add_settings_field( 'username_is_email', 'Use Email as Username', array( $this, 'rpr_checkbox_setting' ), 'rpr-features', 'rpr_form_options', array(
		'label_for' => 'username_is_email',
		'label' => 	__( 'New users will not be asked to enter a username, instead their email address will be used as their username.', 'register-plus-redux' ) ) ); 
	add_settings_field( 'double_check_email', 'Confirm Email', array( $this, 'rpr_checkbox_setting' ), 'rpr-features', 'rpr_form_options', array(
		'label_for' => 'double_check_email',
		'label' => 	__( 'Require new users to enter e-mail address twice during registration.', 'register-plus-redux' ) ) ); 
	add_settings_field( 'default_css', 'Use Default Style Rules', array( $this, 'rpr_checkbox_setting' ), 'rpr-features', 'rpr_form_options', array(
		'label_for' => 'default_css',
		'label' => 	__( 'Apply default Wordpress styling to all fields.', 'register-plus-redux' ) ) ); 
	add_settings_field( 'required_fields_style', 'Required Fields Style Rules', array( $this, 'rpr_textbox_setting' ), 'rpr-features', 'rpr_form_options', array(
		'label_for' => 'required_fields_style' ) ); 
	add_settings_field( 'default_css', 'Use Default Style Rules', array( $this, 'rpr_checkbox_setting' ), 'rpr-features', 'rpr_form_options', array(
		'label_for' => 'default_css',
		'label' => 	__( 'Apply default Wordpress styling to all fields.', 'register-plus-redux' ) ) ); 
}

public function rpr_feature_verify_user_email( $args ) {  
?>
				<?php $options = get_option( 'register_plus_redux_options_v2' ); ?>
				<tr valign="top"><th scope="row">A Checkbox</th>
					<td><input name="test_option[option1]" type="checkbox" value="1" <?php checked('1', $options['option1']); ?> /></td>
				</tr>
				<tr valign="top"><th scope="row">Some text</th>
					<td><input type="text" name="test_option[sometext]" value="<?php echo $options['sometext']; ?>" /></td>
				</tr>
<?php
}

public function rpr_form_registration_options() {  
	echo '<p>', __( 'Select which fields to show on the Registration Form. Users will not be able to register without completing any fields marked required.', 'register-plus-redux' ), '</p>';
}

public function rpr_form_signup_options() {  
	echo '<p>', __( 'Select which fields to show on the Signup Form. Users will not be able to signup without completing any fields marked required.', 'register-plus-redux' ), '</p>';
}

public function rpr_checkbox_setting( $args ) {
	global $register_plus_redux;
	echo '<input type="checkbox" id="', $args['label_for'], '" name="', $args['label_for'], '" value="1" ', checked( $register_plus_redux->rpr_get_option( $args['label_for'] ), 1, false ), '/>';   
	echo '<label for="', $args['label_for'], '"> ', $args['label'], '</label>';   
}

public function rpr_features_page() {
	?>
	<div class="wrap">
		<h2>Register Plus Redux Features</h2>
		<?php settings_errors(); ?>
		<form method="post" action="options.php">
			<?php settings_fields( 'register-plus-redux' ); ?>
			<?php do_settings_sections( 'rpr-features' ); ?>
			<?php submit_button(); ?>
		</form>
	</div>
	<?php	
...
//function rpr_sanitize_options( $value, $option ) {
public function rpr_sanitize_options( $input, $option ) {
	$input['custom_logo_url'] = isset( $input['custom_logo_url'] ) ? esc_url_raw( $input['custom_logo_url'] ) : '';
	if ( isset( $input['remove_logo'] ) ) $input['custom_logo_url'] = '';
	$input['verify_user_email'] = isset( $input['verify_user_email'] ) ? 1 : 0;
	$input['message_verify_user_email'] = isset( $input['message_verify_user_email'] ) ? wp_kses_data( $input['message_verify_user_email'] ) : '';
	$input['verify_user_admin'] = isset( $input['verify_user_admin'] ) ? 1 : 0;
	$input['message_verify_user_admin'] = isset( $input['message_verify_user_admin'] ) ? wp_kses_data( $input['message_verify_user_admin'] ) : '';
	$input['delete_unverified_users_after'] = isset( $input['delete_unverified_users_after'] ) ? absint( $input['delete_unverified_users_after'] ) : 0;
	$input['registration_redirect_url'] = isset( $input['registration_redirect_url'] ) ? esc_url_raw( $input['registration_redirect_url'] ) : '';
	$input['verification_redirect_url'] = isset( $input['verification_redirect_url'] ) ? esc_url_raw( $input['verification_redirect_url'] ) : '';
	$input['autologin_user'] = isset( $input['autologin_user'] ) ? 1 : 0;
	
	$input['username_is_email'] = isset( $input['username_is_email'] ) ? 1 : 0;
	$input['double_check_email'] = isset( $input['double_check_email'] ) ? 1 : 0;
	$input['show_fields'] = ( isset( $input['show_fields'] ) && is_array( $input['show_fields'] ) ) ? $input['show_fields'] : array();
	$input['required_fields'] = ( isset( $input['required_fields'] ) && is_array( $input['required_fields'] ) ) ? $input['required_fields'] : array();
	$input['user_set_password'] = isset( $input['user_set_password'] ) ? 1 : 0;
	$input['min_password_length'] = isset( $input['min_password_length'] ) ? absint( $input['min_password_length'] ) : 0;
	$input['disable_password_confirmation'] = isset( $input['disable_password_confirmation'] ) ? 1 : 0;
	$input['show_password_meter'] = isset( $input['show_password_meter'] ) ? 1 : 0;
	$input['message_empty_password'] = isset( $input['message_empty_password'] ) ? wp_kses_data( $input['message_empty_password'] ) : '';
	$input['message_short_password'] = isset( $input['message_short_password'] ) ? wp_kses_data( $input['message_short_password'] ) : '';
	$input['message_bad_password'] = isset( $input['message_bad_password'] ) ? wp_kses_data( $input['message_bad_password'] ) : '';
	$input['message_good_password'] = isset( $input['message_good_password'] ) ? wp_kses_data( $input['message_good_password'] ) : '';
	$input['message_strong_password'] = isset( $input['message_strong_password'] ) ? wp_kses_data( $input['message_strong_password'] ) : '';
	$input['message_mismatch_password'] = isset( $input['message_mismatch_password'] ) ? wp_kses_data( $input['message_mismatch_password'] ) : '';
	$input['enable_invitation_code'] = isset( $input['enable_invitation_code'] ) ? 1 : 0;
	$input['require_invitation_code'] = isset( $input['require_invitation_code'] ) ? 1 : 0;
	$input['invitation_code_case_sensitive'] = isset( $input['invitation_code_case_sensitive'] ) ? 1 : 0;
	$input['invitation_code_unique'] = isset( $input['invitation_code_unique'] ) ? 1 : 0;
	$input['enable_invitation_tracking_widget'] = isset( $input['enable_invitation_tracking_widget'] ) ? 1 : 0;
	$input['show_disclaimer'] = isset( $input['show_disclaimer'] ) ? 1 : 0;
	$input['message_disclaimer_title'] = isset( $input['message_disclaimer_title'] ) ? sanitize_text_field( $input['message_disclaimer_title'] ) : '';
	$input['message_disclaimer'] = isset( $input['message_disclaimer'] ) ? wp_kses_data( $input['message_disclaimer'] ) : '';
	$input['require_disclaimer_agree'] = isset( $input['require_disclaimer_agree'] ) ? 1 : 0;
	$input['message_disclaimer_agree'] = isset( $input['message_disclaimer_agree'] ) ? sanitize_text_field( $input['message_disclaimer_agree'] ) : '';
	$input['show_license'] = isset( $input['show_license'] ) ? 1 : 0;
	$input['message_license_title'] = isset( $input['message_license_title'] ) ? sanitize_text_field( $input['message_license_title'] ) : '';
	$input['message_license'] = isset( $input['message_license'] ) ? wp_kses_data( $input['message_license'] ) : '';
	$input['require_license_agree'] = isset( $input['require_license_agree'] ) ? 1 : 0;
	$input['message_license_agree'] = isset( $input['message_license_agree'] ) ? sanitize_text_field( $input['message_license_agree'] ) : '';
	$input['show_privacy_policy'] = isset( $input['show_privacy_policy'] ) ? 1 : 0;
	$input['message_privacy_policy_title'] = isset( $input['message_privacy_policy_title'] ) ? sanitize_text_field( $input['message_privacy_policy_title'] ) : '';
	$input['message_privacy_policy'] = isset( $input['message_privacy_policy'] ) ? wp_kses_data( $input['message_privacy_policy'] ) : '';
	$input['require_privacy_policy_agree'] = isset( $input['require_privacy_policy_agree'] ) ? 1 : 0;
	$input['message_privacy_policy_agree'] = isset( $input['message_privacy_policy_agree'] ) ? sanitize_text_field( $input['message_privacy_policy_agree'] ) : '';
	$input['default_css'] = isset( $input['default_css'] ) ? 1 : 0;
	if ( isset( $input['required_fields_style'] ) ) {
		...
		$required_fields_style = 'div {' . $input['required_fields_style'] . '}';
		...
		if ( !empty( $matches[1] ) ) $input['required_fields_style'] = $matches[1];
	}
	$input['required_fields_asterisk'] = isset( $input['required_fields_asterisk'] ) ? 1 : 0;
	$input['starting_tabindex'] = isset( $input['starting_tabindex'] ) ? absint( $input['starting_tabindex'] ) : 0;
	
	/*
	if ( isset( $input['datepicker_firstdayofweek'] ) ) $input['datepicker_firstdayofweek'] = absint( $input['datepicker_firstdayofweek'] );
	if ( isset( $input['datepicker_dateformat'] ) ) $input['datepicker_dateformat'] = sanitize_text_field( $input['datepicker_dateformat'] );
	if ( isset( $input['datepicker_startdate'] ) ) $input['datepicker_startdate'] = sanitize_text_field( $input['datepicker_startdate'] );
	if ( isset( $input['datepicker_calyear'] ) ) $input['datepicker_calyear'] = sanitize_text_field( $input['datepicker_calyear'] );
	if ( isset( $input['datepicker_calmonth'] ) ) $input['datepicker_calmonth'] = sanitize_text_field( $input['datepicker_calmonth'] );
	*/
	
	$input['disable_user_message_registered'] = isset( $input['disable_user_message_registered'] ) ? 1 : 0;
	$input['disable_user_message_created'] = isset( $input['disable_user_message_created'] ) ? 1 : 0;
	$input['custom_user_message'] = isset( $input['custom_user_message'] ) ? 1 : 0;
	$input['user_message_from_email'] = isset( $input['user_message_from_email'] ) ? sanitize_text_field( $input['user_message_from_email'] ) : '';
	$input['user_message_from_name'] = isset( $input['user_message_from_name'] ) ? sanitize_text_field( $input['user_message_from_name'] ) : '';
	$input['user_message_subject'] = isset( $input['user_message_subject'] ) ? sanitize_text_field( $input['user_message_subject'] ) : '';
	$input['user_message_body'] = isset( $input['user_message_body'] ) ? wp_kses_data( $input['user_message_body'] ) : '';
	$input['send_user_message_in_html'] = isset( $input['send_user_message_in_html'] ) ? 1 : 0;
	$input['user_message_newline_as_br'] = isset( $input['user_message_newline_as_br'] ) ? 1 : 0;
	$input['custom_verification_message'] = isset( $input['custom_verification_message'] ) ? 1 : 0;
	$input['verification_message_from_email'] = isset( $input['verification_message_from_email'] ) ? sanitize_text_field( $input['verification_message_from_email'] ) : '';
	$input['verification_message_from_name'] = isset( $input['verification_message_from_name'] ) ? sanitize_text_field( $input['verification_message_from_name'] ) : '';
	$input['verification_message_subject'] = isset( $input['verification_message_subject'] ) ? sanitize_text_field( $input['verification_message_subject'] ) : '';
	$input['verification_message_body'] = isset( $input['verification_message_body'] ) ? wp_kses_data( $input['verification_message_body'] ) : '';
	$input['send_verification_message_in_html'] = isset( $input['send_verification_message_in_html'] ) ? 1 : 0;
	$input['verification_message_newline_as_br'] = isset( $input['verification_message_newline_as_br'] ) ? 1 : 0;
	
	$input['disable_admin_message_registered'] = isset( $input['disable_admin_message_registered'] ) ? 1 : 0;
	$input['disable_admin_message_created'] = isset( $input['disable_admin_message_created'] ) ? 1 : 0;
	$input['admin_message_when_verified'] = isset( $input['admin_message_when_verified'] ) ? 1 : 0;
	$input['custom_admin_message'] = isset( $input['custom_admin_message'] ) ? 1 : 0;
	$input['admin_message_from_email'] = isset( $input['admin_message_from_email'] ) ? sanitize_text_field( $input['admin_message_from_email'] ) : '';
	$input['admin_message_from_name'] = isset( $input['admin_message_from_name'] ) ? sanitize_text_field( $input['admin_message_from_name'] ) : '';
	$input['admin_message_subject'] = isset( $input['admin_message_subject'] ) ? sanitize_text_field( $input['admin_message_subject'] ) : '';
	$input['admin_message_body'] = isset( $input['admin_message_body'] ) ? wp_kses_data( $input['admin_message_body'] ) : '';
	$input['send_admin_message_in_html'] = isset( $input['send_admin_message_in_html'] ) ? 1 : 0;
	$input['admin_message_newline_as_br'] = isset( $input['admin_message_newline_as_br'] ) ? 1 : 0;
	
	$input['custom_registration_page_css'] = '';
	if ( isset( $input['custom_registration_page_css'] ) ) {
		...
		$custom_registration_page_css = $input['custom_registration_page_css'];
		...
		$input['custom_registration_page_css'] = $csstidy->print->plain();
	}
	
	$input['custom_login_page_css'] = '';
	if ( isset( $input['custom_login_page_css'] ) ) {
		// Stolen from Jetpack 2.0.4 custom-css.php Jetpack_Custom_CSS::init()
		require_once( 'csstidy/class.csstidy.php' );
		$csstidy = new csstidy();
		$csstidy->set_cfg( 'remove_bslash', false );
		...
		$csstidy->set_cfg( 'preserve_css', true );
		$csstidy->set_cfg( 'template', dirname( __FILE__ ) . '/csstidy/wordpress-standard.tpl' );
		$custom_login_page_css = $input['custom_login_page_css'];
		...
		$input['custom_login_page_css'] = $csstidy->print->plain();
	}
			...
			<form method="post">
							...
							<table>
								...
								<tbody>
									...
									<tr valign="center">
										...
										<td align="center" style="padding-top: 0px; padding-bottom: 0px;"><input type="checkbox" name="required_fields[]" id="required_fields[]" alue="about" <?php if ( is_array( $register_plus_redux->rpr_get_option( 'required_fields' ) ) && in_array( 'about', $register_plus_redux->rpr_get_option( 'required_fields' ) ) ) echo 'checked="checked"'; ?> <?php if ( is_array( $register_plus_redux->rpr_get_option( 'show_fields' ) ) && !in_array( 'about', $register_plus_redux->rpr_get_option( 'show_fields' ) ) ) echo 'disabled="disabled"'; ?> /></td>
									</tr>
								</tbody>
							</table>
						</td>
				...
				<table id="meta_fields" style="padding-left: 0px; width: 90%;">
					<tbody class="fields">
						...
						foreach ( $redux_usermeta as $index => $meta_field ) {
							...
							echo "\n", '<tr class="label"><td><img src="', plugins_url( 'images\arrow-move.png', __FILE__ ), '" alt="', esc_attr__( 'Reorder', 'register-plus-redux' ), '" title="', esc_attr__( 'Drag to Reorder', 'register-plus-redux' ), '" class="sortHandle" style="cursor: move;" />&nbsp;<input type="text" name="label[', $index, ']" id="label[', $index, ']" value="', esc_attr( $meta_field['label'] ), '" />&nbsp;<span class="enableDisableFieldSettings" style="color:#0000FF; cursor: pointer;">Show Settings</span></td></tr>';
				...
				*/ ?>
				...
				<p class="submit">
					...
					<input type="button" class="button" value="<?php esc_attr_e( 'Preview Registration Page', 'register-plus-redux' ); ?>" name="preview" id="preview" onclick="window.open('<?php echo wp_login_url(), '?action=register'; ?>');" />
				</p>
			</form>
			</div>
			<?php
...
if ( class_exists( 'RPR_Admin_Menu_wip' ) ) $rpr_admin_menu_wip = new RPR_Admin_Menu_wip();
?>